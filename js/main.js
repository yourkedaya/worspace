...
(function() {
    ...
    function renderTools() {
        toolsGrid.innerHTML = TOOLS.map((t, idx) => {
            ...
            const dataAction = t.action || '';
            return `
                <button class="tool-card${featuredClass}" data-action="${dataAction}" data-idx="${idx}">
        ...
        // Delegate click — safer than inline onclick
        toolsGrid.querySelectorAll('.tool-card').forEach(btn => {
            btn.addEventListener('click', () => {
                const action = btn.dataset.action;
                const name = TOOLS[btn.dataset.idx]?.name || '该工具';
                if (action === 'openMatcher') {
                    if (window.MatcherUI && typeof window.MatcherUI.open === 'function') {
                        window.MatcherUI.open();
                    } else {
                        showToast('表格匹配器加载失败，请刷新页面或联系管理员检查 matcher.js 是否已上传');
                    }
                } else {
                    showToast(name + ' 即将上线');
                }
